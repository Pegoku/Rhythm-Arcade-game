...
#define LED3_PIN 18
#define LED4_PIN 19
#define LED5_PIN 15   // Add new LED strips
#define LED6_PIN 13
...
#define BUTTON5_PIN 12  // Add new button pins
...
CRGB leds3[NUM_LEDS];
CRGB leds4[NUM_LEDS];
CRGB leds5[NUM_LEDS];  // Add new LED arrays
CRGB leds6[NUM_LEDS];
...
// Song structure
struct Note {
  int stripIndex;      // 0-5 for strips, -1 for blank/pause
...
// Tone frequencies for each button - 6 note scale (C major pentatonic + extras)
const int toneFreqs[6] = {262, 330, 392, 523, 659, 784}; // C4, E4, G4, C5, E5, G5

// Map musical notes to button indices (improved mapping with 6 buttons)
int getNoteButton(char note, bool isFlat = false) {
  switch(note) {
    case 'C': return 0;  // C4 - 262Hz
    case 'D': return 1;  // D->E4 - 330Hz (closest available)
    case 'E': return 1;  // E4 - 330Hz
    case 'F': return 2;  // F->G4 - 392Hz (closest available)
    case 'G': return 2;  // G4 - 392Hz
    case 'A': return 3;  // A->C5 - 523Hz (octave up)
    case 'B': return 4;  // B->E5 - 659Hz (closest high note)
    default: return 0;
  }
}
...
Note song2[] = {
  ...
  {getNoteButton('B'), 0, 125},     // Bb (eighth note)
  {getNoteButton('G'), 500, 250},   // G (quarter note)  
  {getNoteButton('G'), 1000, 125},  // G (eighth note)
  {getNoteButton('D'), 1250, 125},  // D (eighth note)
  {getNoteButton('D'), 1500, 125},  // D (eighth note)
  {getNoteButton('A'), 1750, 250},  // A (quarter note)
  {getNoteButton('F'), 2250, 250},  // F (quarter note)
  {getNoteButton('F'), 2750, 125},  // F (eighth note)
  {getNoteButton('D'), 3000, 250},  // D (quarter note)
  
  // Measure 2: D A F F C C E E F
  {getNoteButton('D'), 4000, 125},  // D (eighth note)
  {getNoteButton('A'), 4250, 250},  // A (quarter note)
  {getNoteButton('F'), 4750, 250},  // F (quarter note)
  {getNoteButton('F'), 5250, 125},  // F (eighth note)
  {getNoteButton('C'), 5500, 125},  // C (eighth note)
  {getNoteButton('C'), 5750, 125},  // C (eighth note)
  {getNoteButton('E'), 6000, 125},  // E (eighth note)
  {getNoteButton('E'), 6250, 125},  // E (eighth note)
  {getNoteButton('F'), 6500, 250},  // F (quarter note)
  
  // Measure 3: D Bb G G D D A F F D
  {getNoteButton('D'), 8000, 250},  // D (quarter note)
  {getNoteButton('B'), 8500, 125},  // Bb (eighth note)
  {getNoteButton('G'), 8750, 250},  // G (quarter note)
  {getNoteButton('G'), 9250, 125},  // G (eighth note)
  {getNoteButton('D'), 9500, 125},  // D (eighth note)
  {getNoteButton('D'), 9750, 125},  // D (eighth note)
  {getNoteButton('A'), 10000, 250}, // A (quarter note)
  {getNoteButton('F'), 10500, 250}, // F (quarter note)
  {getNoteButton('F'), 11000, 125}, // F (eighth note)
  {getNoteButton('D'), 11250, 250}, // D (quarter note)
  
  // Measure 4: D A F F C C E E F
  {getNoteButton('D'), 12000, 125}, // D (eighth note)
  {getNoteButton('A'), 12250, 250}, // A (quarter note)
  {getNoteButton('F'), 12750, 250}, // F (quarter note)
  {getNoteButton('F'), 13250, 125}, // F (eighth note)
  {getNoteButton('C'), 13500, 125}, // C (eighth note)
  {getNoteButton('C'), 13750, 125}, // C (eighth note)
  {getNoteButton('E'), 14000, 125}, // E (eighth note)
  {getNoteButton('E'), 14250, 125}, // E (eighth note)
  {getNoteButton('F'), 14500, 250}, // F (quarter note)

  // Measure 5: D Bb G G D D A F F D (half notes)
  {getNoteButton('D'), 16000, 500}, // D (half note)
  {getNoteButton('B'), 17000, 125}, // Bb (eighth note)
  {getNoteButton('G'), 17500, 500}, // G (half note)
  {getNoteButton('G'), 18500, 125}, // G (eighth note)
  {getNoteButton('D'), 18750, 125}, // D (eighth note)
  {getNoteButton('D'), 19000, 125}, // D (eighth note)
  {getNoteButton('A'), 19250, 250}, // A (quarter note)
  {getNoteButton('F'), 19750, 500}, // F (half note)
  {getNoteButton('F'), 20750, 125}, // F (eighth note)
  {getNoteButton('D'), 21000, 250}, // D (quarter note)

  // Measure 6: D A F F C C E E F
  {getNoteButton('D'), 22000, 125}, // D (eighth note)
  {getNoteButton('A'), 22250, 250}, // A (quarter note)
  {getNoteButton('F'), 22750, 500}, // F (half note)
  {getNoteButton('F'), 23750, 125}, // F (eighth note)
  {getNoteButton('C'), 24000, 250}, // C (quarter note)
  {getNoteButton('C'), 24500, 125}, // C (eighth note)
  {getNoteButton('E'), 24750, 250}, // E (quarter note)
  {getNoteButton('E'), 25250, 125}, // E (eighth note)
  {getNoteButton('F'), 25500, 500}, // F (half note)
  
  // Measure 7: D Bb G G D D A F F D (final measure)
  {getNoteButton('D'), 27000, 250}, // D (quarter note)
  {getNoteButton('B'), 27500, 125}, // Bb (eighth note)
  {getNoteButton('G'), 27750, 250}, // G (quarter note)
  {getNoteButton('G'), 28250, 125}, // G (eighth note)
  {getNoteButton('D'), 28500, 125}, // D (eighth note)
  {getNoteButton('D'), 28750, 125}, // D (eighth note)
  {getNoteButton('A'), 29000, 250}, // A (quarter note)
  {getNoteButton('F'), 29500, 500}, // F (half note)
  {getNoteButton('F'), 30500, 125}, // F (eighth note)
  {getNoteButton('D'), 30750, 500}, // D (half note)
...
bool buttonProcessed[6] = {false, false, false, false, false, false};
bool noteWasActive[6] = {false, false, false, false, false, false}; // Track if note was active in previous frame
bool noteWasPressed[6] = {false, false, false, false, false, false}; // Track if note was successfully pressed
...
void setup()
{
  ...
  FastLED.addLeds<LED_TYPE, LED5_PIN, GRB>(leds5, NUM_LEDS).setCorrection(TypicalLEDStrip);
  FastLED.addLeds<LED_TYPE, LED6_PIN, GRB>(leds6, NUM_LEDS).setCorrection(TypicalLEDStrip);
...
void playRandomMode() {
  ...
  int stripIndex = random(0, 6);  // Now 6 strips
  CRGB* currentLeds;
  uint8_t buttonPin;
  CRGB activeColor;
  ...
  switch (stripIndex) {
    ...
    case 4:
      currentLeds = leds5;
      buttonPin = BUTTON5_PIN;
      activeColor = CRGB::Purple;
      break;
    case 5:
      currentLeds = leds6;
      buttonPin = BUTTON6_PIN;
      activeColor = CRGB::Orange;
      break;
  }
...
void playSongMode() {
  ...
  if (!songActive) {
    ...
    for (int i = 0; i < 6; i++) {  // Now 6 buttons
  ...
  bool noteCurrentlyActive[6] = {false, false, false, false, false, false}; // Track which notes are active this frame
