...
// Map musical notes to button indices (based on frequency range)
// Lower notes = button 0, higher notes = button 3
int getNoteButton(char note, bool isFlat = false) {
  switch(note) {
    case 'D': return 0;  // Low D
    case 'E': return 0;  // Low E  
    case 'F': return 1;  // Mid F
    case 'G': return 2;  // Mid G
    case 'A': return 2;  // Mid A
    case 'B': return 3;  // High B
    case 'C': return 1;  // Mid C
    default: return 0;
  }
}
...
Note song2[] = {
  ...
  {getNoteButton('B'), 0, 250},     // Bb (eighth note)
  {getNoteButton('G'), 250, 500},   // G (quarter note)  
  {getNoteButton('G'), 750, 250},   // G (eighth note)
  {getNoteButton('D'), 1000, 500},  // D (quarter note)
  {getNoteButton('D'), 1500, 250},  // D (eighth note)
  {getNoteButton('A'), 1750, 500},  // A (quarter note)
  {getNoteButton('F'), 2250, 500},  // F (quarter note)
  {getNoteButton('F'), 2750, 250},  // F (eighth note)
  {getNoteButton('D'), 3000, 500},  // D (quarter note)
  
  // Measure 2: D A F F C C E E F
  {getNoteButton('D'), 3500, 250},  // D (eighth note)
  {getNoteButton('A'), 3750, 500},  // A (quarter note)
  {getNoteButton('F'), 4250, 500},  // F (quarter note)
  {getNoteButton('F'), 4750, 250},  // F (eighth note)
  {getNoteButton('C'), 5000, 500},  // C (quarter note)
  {getNoteButton('C'), 5500, 250},  // C (eighth note)
  {getNoteButton('E'), 5750, 500},  // E (quarter note)
  {getNoteButton('E'), 6250, 250},  // E (eighth note)
  {getNoteButton('F'), 6500, 500},  // F (quarter note)
  
  // Measure 3: D Bb G G D D A F F D
  {getNoteButton('D'), 7000, 250},  // D (eighth note)
  {getNoteButton('B'), 7250, 250},  // Bb (eighth note)
  {getNoteButton('G'), 7500, 500},  // G (quarter note)
  {getNoteButton('G'), 8000, 250},  // G (eighth note)
  {getNoteButton('D'), 8250, 500},  // D (quarter note)
  {getNoteButton('D'), 8750, 250},  // D (eighth note)
  {getNoteButton('A'), 9000, 500},  // A (quarter note)
  {getNoteButton('F'), 9500, 500},  // F (quarter note)
  {getNoteButton('F'), 10000, 250}, // F (eighth note)
  {getNoteButton('D'), 10250, 500}, // D (quarter note)
  
  // Measure 4: D A F F C C E E F
  {getNoteButton('D'), 10750, 250}, // D (eighth note)
  {getNoteButton('A'), 11000, 500}, // A (quarter note)
  {getNoteButton('F'), 11500, 500}, // F (quarter note)
  {getNoteButton('F'), 12000, 250}, // F (eighth note)
  {getNoteButton('C'), 12250, 500}, // C (quarter note)
  {getNoteButton('C'), 12750, 250}, // C (eighth note)
  {getNoteButton('E'), 13000, 500}, // E (quarter note)
  {getNoteButton('E'), 13500, 250}, // E (eighth note)
  {getNoteButton('F'), 13750, 500}, // F (quarter note)
  
  // Measure 5: D Bb G G D D A F F D (whole notes/half notes)
  {getNoteButton('D'), 14250, 500}, // D
  {getNoteButton('B'), 14750, 250}, // Bb (dotted)
  {getNoteButton('G'), 15250, 750}, // G (dotted quarter)
  {getNoteButton('G'), 16000, 500}, // G
  {getNoteButton('D'), 16500, 250}, // D
  {getNoteButton('D'), 16750, 500}, // D
  {getNoteButton('A'), 17250, 750}, // A (dotted quarter)
  {getNoteButton('F'), 18000, 1000},// F (half note)
  {getNoteButton('F'), 19000, 500}, // F
  {getNoteButton('D'), 19500, 500}, // D
  
  // Continue pattern for measures 6-7 (similar to 5)
  {getNoteButton('D'), 20000, 250}, // D
  {getNoteButton('A'), 20250, 500}, // A
  {getNoteButton('F'), 20750, 1000},// F (half note)
  {getNoteButton('F'), 21750, 500}, // F
  {getNoteButton('C'), 22250, 500}, // C
  {getNoteButton('C'), 22750, 250}, // C
  {getNoteButton('E'), 23000, 750}, // E (dotted quarter)
  {getNoteButton('E'), 23750, 250}, // E
  {getNoteButton('F'), 24000, 1000},// F (half note)
  
  {-1, 25000, 1000}, // End marker
};
