...
uint8_t lastButtonStates = 0; // Store last 8 button states
...
{
  Wire.requestFrom(IO_EXPANDER_ADDRESS, (uint8_t)1); // Request 1 byte from the IO expander
  if (Wire.available())
  {
    return Wire.read();   // Read the first byte (LSB)
    // If you want to support 16 buttons, read the second byte as well and use uint16_t
  }
...
void loop()
{
  uint8_t currentStates = readButtonStates(); // Read all 8 button states
  for (uint8_t buttonNumber = 0; buttonNumber < 8; buttonNumber++) // Iterate through all 8 buttons
  {
