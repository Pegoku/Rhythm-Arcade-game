...
Note song1[] = {
  {0, 0,0 1000},     // Red starts at 0ms, lasts 1000ms
  {1, 5000, 800},    // Green starts at 500ms, lasts 800ms (overlaps with red)
  {2, 12000, 600},   // Blue starts at 1200ms, lasts 600ms
  {3, 15000, 700},   // Yellow starts at 1500ms, lasts 700ms (overlaps with blue)
  {0, 25000, 500},   // Red again at 2500ms
  {1, 28000, 600},   // Green at 2800ms (overlaps with red)
  {-1, 35000, 500},  // End marker/pause
};
...
void playSongMode() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (noteWasActive[i] && !noteCurrentlyActive[i] && !buttonProcessed[i]) {
    ...
    // Reset button processing when note ends
