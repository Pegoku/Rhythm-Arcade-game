...
void loop()
{
  ...
  */
  ...
  if (digitalRead(BUTTON1_PIN) == LOW) { // Up button
  ...
  if (digitalRead(BUTTON2_PIN) == LOW) { // Down button
    characters[selectedIndex]--;
  ...
  if (digitalRead(BUTTON3_PIN) == LOW) { // Right button
    selectedIndex = (selectedIndex + 1) % 4; // Move to the next character
    updateDisplay2();
    delay(200); // Debounce delay
  }

  if (digitalRead(BUTTON4_PIN) == LOW) { // Left button
